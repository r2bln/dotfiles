...
| `shell-env` | добавляет в `~/.bashrc` (если ещё нет) `~/.local/bin` в `PATH`, `EDITOR`/`VISUAL`/`SYSTEMD_EDITOR=nvim`, алиас `e='$EDITOR'` и `sudo='sudo '` (пробел в конце — чтобы `sudo` тоже разворачивал алиасы следующего слова, иначе `sudo e file` падает с «команда не найдена»); также пишет те же переменные в `~/.config/environment.d/dotfiles-editor.conf`, чтобы их видели и программы, запущенные не из bash-шелла (GUI-лаунчер, `systemctl --user edit`, systemd `--user` юниты). Для `sudo systemctl edit` этого недостаточно — sudo по умолчанию сбрасывает окружение, нужен `Defaults env_keep += "EDITOR VISUAL"` в sudoers |
